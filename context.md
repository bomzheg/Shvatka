...
| **Hint part** | Часть подсказки | One piece of a hint's content: text, photo, video, audio, document, GPS point, venue, contact, sticker, rich message… A hint is a list of parts. | `hints.AnyHint`, `enums.HintType` |
| **Rich message** | Форматированное сообщение | A hint part written as a whole small document — headings, lists, tables, quotes and embedded files — carried as its markup (Rich HTML or Rich Markdown) and sent through `sendRichMessage`. | `hints.RichHint`, `enums.RichFormat` |
...
| **Taking a slot** | Взять дату | An author claiming a date, declaring whether the game will be authored by them or by their team. It records intent, not exclusive access. | `TakeSlotInteractor` |
