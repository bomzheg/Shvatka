...
class SeasonCalendar(Calendar):
    ...
    The base widget renders a bare number per day; this one hands the days view
    a `date_text` that reads the getter's marks, and pins the view to the
    season's year so no month outside it is reachable.
    """
    ...
    def _init_views(self) -> dict[CalendarScope, CalendarScopeView]:
        return {
            CalendarScope.DAYS: CalendarDaysView(
                ...
                date_text=SlotMark(),
                today_text=SlotMark(today=True),
            ),
