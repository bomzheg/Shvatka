...
@pytest.mark.asyncio
async def test_another_author_may_re_assign_a_taken_date(
    client: AsyncClient,
    harry: dto.Player,
    harry_token: Token,
    draco: dto.Player,
    ...
    # taking a date somebody else took is a re-assignment, not a conflict
    resp = await take(client, draco_token, slot_id)

    assert resp.is_success
    slot = await check_dao.season_slot.get_slot(slot_id)
    assert slot.owner is not None
    assert slot.owner.id == draco.id
    season = await check_dao.season.get_season(YEAR)
    assert season is not None
    changes = await check_dao.season_change.get_unpublished_changes(season.id)
