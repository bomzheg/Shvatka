...
async def test_write_level(
    ...
    assert "Таймеры" in actual_kb[1][0].text
    ...
    actual_kb = new_message.reply_markup.inline_keyboard
    assert "Ключи" in actual_kb[0][0].text
    assert "Ключи с эффектами" in actual_kb[1][0].text
    assert "Таймеры" in actual_kb[2][0].text
    assert "Подсказки" in actual_kb[3][0].text
    ...
    assert "Готово, сохранить" in msg.reply_markup.inline_keyboard[4][0].text
